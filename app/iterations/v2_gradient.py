...
# Page configuration
st.set_page_config(
    page_title="BetaSense v2 - What-If Analysis",
    page_icon="🏥",
    layout="wide",
...
def main():
    ...
    st.markdown('<p class="sub-header">Predicting Insulin Resistance from Lifestyle & Biomarkers</p>', unsafe_allow_html=True)
    ...
    page = st.sidebar.radio(
        "Choose a page:",
        ["🩺 Risk Calculator", "🔮 What-If Analysis", "🔬 About the Science"]
    )
    ...
    if page == "🩺 Risk Calculator":
        ...
        if st.button("🔍 Calculate Risk", type="primary", use_container_width=True):
            ...
            try:
                ...
                st.write("See how changing one factor affects your predicted insulin resistance.")

                sweep_var = st.selectbox("Choose a variable to explore:", [
                    "Sugar Intake (g/day)",
                ...
                sweep_config = {
                    "Sugar Intake (g/day)": ("DR1TSUGR", np.linspace(0, 150, 30)),
                    "Physical Activity (min/week)": ("comprehensive_inactivity_score", np.linspace(0, 100, 30)),
                    "CRP Level (mg/L)": ("LBXHSCRP", np.linspace(0.1, 15, 30)),
                    "Fiber Intake (g/day)": ("DR1TFIBE", np.linspace(5, 50, 30))
                }

                var_name, sweep_range = sweep_config[sweep_var]

                # Run model across the sweep range
                sweep_results = []
                for val in sweep_range:
                    sweep_data = input_data_full.copy()
                    if sweep_var == "Physical Activity (min/week)":
                        sweep_data['comprehensive_inactivity_score'] = val
                        sweep_data['sugar_inactivity_interaction'] = sweep_data['DR1TSUGR'] * val / 100
                        sweep_data['bmi_inactivity_interaction'] = sweep_data['BMXBMI'] * val / 100
                    else:
                        sweep_data[var_name] = val
                        if var_name == 'DR1TSUGR':
                            sweep_data['sugar_inactivity_interaction'] = val * sweep_data['comprehensive_inactivity_score'] / 100
                            sweep_data['sugar_crp_interaction'] = val * sweep_data['LBXHSCRP']
                        elif var_name == 'LBXHSCRP':
                            sweep_data['sugar_crp_interaction'] = sweep_data['DR1TSUGR'] * val
                            sweep_data['crp_bmi_interaction'] = val * sweep_data['BMXBMI']
                            sweep_data['crp_squared'] = val ** 2

                    sweep_df = pd.DataFrame([sweep_data])[feature_order_homa_ir]
                    pred = homa_ir_model.predict(sweep_df)[0]
                    sweep_results.append(pred)

                # Build display x-axis
                if sweep_var == "Physical Activity (min/week)":
                    display_x = [420 - (v / 100 * 420) for v in sweep_range]
                    x_label = "Activity (min/week)"
                else:
                    display_x = sweep_range.tolist()
                    x_label = sweep_var

                import plotly.graph_objects as go_sweep
                fig_sweep = go_sweep.Figure()
                fig_sweep.add_trace(go_sweep.Scatter(
                    x=display_x, y=sweep_results,
                    mode='lines+markers',
                    line=dict(color='#1f77b4', width=3),
                    marker=dict(size=4),
                    name='Predicted HOMA-IR'
                ))
                fig_sweep.add_hline(y=2.5, line_dash="dash", line_color="red",
                                    annotation_text="Risk Threshold (2.5)")
                # Mark current value
                if sweep_var == "Physical Activity (min/week)":
                    current_x = 420 - (inactivity_score / 100 * 420)
                elif var_name == 'DR1TSUGR':
                    current_x = sugar_intake
                elif var_name == 'LBXHSCRP':
                    current_x = crp_level
                else:
                    current_x = fiber_intake
                fig_sweep.add_vline(x=current_x, line_dash="dot", line_color="green",
                                    annotation_text="You are here")
                fig_sweep.update_layout(
                    title=f"How {sweep_var} Affects Your Insulin Resistance",
                    xaxis_title=x_label,
                    yaxis_title="Predicted HOMA-IR",
                    height=400,
                    template="plotly_white"
                )
                st.plotly_chart(fig_sweep, use_container_width=True)
                st.info("This shows your **actual model prediction** across different values — not a simplified estimate.")
    ...
    # =========================================================================
    # PAGE 2: WHAT-IF ANALYSIS (standalone page)
    # =========================================================================
    elif page == "🔮 What-If Analysis":
        st.title("🔮 What-If Sensitivity Analysis")
        st.write("Explore how changing a single lifestyle or biomarker variable affects predicted insulin resistance.")
        st.write("This tool sweeps one variable across its full range while holding everything else constant, "
                 "running the trained XGBoost model at each point.")

        st.markdown("---")

        # Use default/median values for the sweep
        sweep_var = st.selectbox("Choose a variable to sweep:", [
            "Sugar Intake (g/day)",
            "Physical Activity (min/week)",
            "CRP Level (mg/L)",
            "Fiber Intake (g/day)"
        ...
        baseline = {
            'comprehensive_inactivity_score': 64.3,
            'DR1TSUGR': 50.0, 'DR1TFIBE': 15.0, 'LBXHSCRP': 1.0,
            'BMXBMI': 22.0, 'BMXWAIST': 75.0, 'LBXGH': 5.3,
            'BPXSY2': 112.0, 'BPXDI2': 65.0, 'carb_percent': 52.0,
            'nlr': 1.8, 'RIDAGEYR': 15, 'RIAGENDR': 1,
            'sugar_inactivity_interaction': 50.0 * 64.3 / 100,
            'bmi_inactivity_interaction': 22.0 * 64.3 / 100,
            'sugar_crp_interaction': 50.0 * 1.0,
            'crp_bmi_interaction': 1.0 * 22.0,
            'bmi_squared': 22.0 ** 2, 'crp_squared': 1.0 ** 2
        }

        sweep_config = {
            "Sugar Intake (g/day)": ("DR1TSUGR", np.linspace(0, 150, 40)),
            "Physical Activity (min/week)": ("comprehensive_inactivity_score", np.linspace(0, 100, 40)),
            "CRP Level (mg/L)": ("LBXHSCRP", np.linspace(0.1, 15, 40)),
            "Fiber Intake (g/day)": ("DR1TFIBE", np.linspace(5, 50, 40))
        }
        ...
        for val in sweep_range:
            d = baseline.copy()
            if sweep_var == "Physical Activity (min/week)":
                d['comprehensive_inactivity_score'] = val
                d['sugar_inactivity_interaction'] = d['DR1TSUGR'] * val / 100
                d['bmi_inactivity_interaction'] = d['BMXBMI'] * val / 100
            else:
                d[var_name] = val
                if var_name == 'DR1TSUGR':
                    d['sugar_inactivity_interaction'] = val * d['comprehensive_inactivity_score'] / 100
                    d['sugar_crp_interaction'] = val * d['LBXHSCRP']
                elif var_name == 'LBXHSCRP':
                    d['sugar_crp_interaction'] = d['DR1TSUGR'] * val
                    d['crp_bmi_interaction'] = val * d['BMXBMI']
                    d['crp_squared'] = val ** 2
            df_sweep = pd.DataFrame([d])[feature_order]
            results.append(homa_ir_model.predict(df_sweep)[0])
        ...
        import plotly.graph_objects as go_page
        fig = go_page.Figure()
        fig.add_trace(go_page.Scatter(x=display_x, y=results, mode='lines+markers',
                                       line=dict(color='#1f77b4', width=3), marker=dict(size=5)))
        fig.add_hline(y=2.5, line_dash="dash", line_color="red", annotation_text="Risk Threshold (2.5)")
        fig.update_layout(title=f"How {sweep_var} Affects Predicted HOMA-IR",
                          xaxis_title=x_label, yaxis_title="Predicted HOMA-IR",
                          height=500, template="plotly_white")
        st.plotly_chart(fig, use_container_width=True)

        st.info("Each point is a **real model prediction** — not an approximation. "
                "All other inputs are held at population median values.")
